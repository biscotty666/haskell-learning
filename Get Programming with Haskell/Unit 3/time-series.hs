...
-- Interesting implementation but does not allow for inerpolating
-- values and so the smoothing function results in
-- too many NA values
...
-- extendedValues contains Maybes
...
-- note the second series will overwrite duplicate key/values from the first
...
--  mconcat [ts1,ts2] produces the same output as above

tsAll :: TS Double
tsAll = mconcat [ts1,ts2,ts3,ts4]

-- use Real in order to be able to use realToFrac
...
-- `isJust` and `fromJust` are from `Data.Maybe`
...
-- 61% growth over the time period
